		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
			...
			unique = val+el.id; //unique identifier
 			...
 			out += "<div class='graphBar"+el.id+"' id='graphFieldBar"+unique+"' style='background-color:"+color+";position: relative; overflow: visible;'></div>";
			...
			//creating legend array from lbl if there is no legends param
 			...
 			if(arr.animate){
 				...
 				$('#graphFieldBar'+unique).animate({'height': fieldHeight},arr.speed*1000,function(){
 					$('.barT').animate({'opacity':1},'slow');
 				});
 			} else {
